    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        save.getValueObservers().add(new Function2<String, BaseFormElement<String>, Unit>() {
            ...
            public Unit invoke(String newValue, BaseFormElement<String> element) {
                ...
                editor.putString("MID_Name", AESEncryption.encrypt(name.getValue()));
                editor.putString("MID_Birthday", AESEncryption.encrypt(dob.getValueAsString()));
                editor.putString("MID_Address1", AESEncryption.encrypt(street.getValue()));
                editor.putString("MID_Address2", AESEncryption.encrypt(street2.getValue()));
                editor.putString("MID_Gender", AESEncryption.encrypt(gender.getValueAsString()));
                editor.putString("MID_City", AESEncryption.encrypt(city.getValue()));
                editor.putString("MID_State", AESEncryption.encrypt(state.getValueAsString()));
                editor.putString("MID_Zip", AESEncryption.encrypt(zipCode.getValue()));
                editor.putString("MID_HomePhone", AESEncryption.encrypt(homePhone.getValue()));
                editor.putString("MID_WorkPhone", AESEncryption.encrypt(workPhone.getValue()));


                editor.putString("MID_Weight", AESEncryption.encrypt(weight.getValue()));
                editor.putString("MID_Height", AESEncryption.encrypt(height.getValue()));
                editor.putString("MID_BloodType", AESEncryption.encrypt(bloodType.getValueAsString()));
                editor.putString("MID_Ethnicity", AESEncryption.encrypt(ethnicity.getValueAsString()));
                editor.putString("MID_MaritalStatus", AESEncryption.encrypt(maritalStatus.getValueAsString()));
                editor.putString("MID_PrimaryInsurance", AESEncryption.encrypt(primaryInsurance.getValueAsString()));
                editor.putString("MID_PrimaryInsuranceNumber", AESEncryption.encrypt(primaryInsuranceNumber.getValueAsString()));
                editor.putString("MID_PrimaryInsuranceGroupNumberOrMainPH", AESEncryption.encrypt(primaryInsuranceGroupNumberOrMainPH.getValueAsString()));
        ...
        SharedPreferences prefs = getSharedPreferences("edu.oaklandstudent.medicalid", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = prefs.edit();
        name.setValue(AESEncryption.decrypt(prefs.getString("MID_Name", null)));
        street.setValue(AESEncryption.decrypt(prefs.getString("MID_Address1", null)));
        street2.setValue(AESEncryption.decrypt(prefs.getString("MID_Address2", null)));
        city.setValue(AESEncryption.decrypt(prefs.getString("MID_City", null)));
        zipCode.setValue(AESEncryption.decrypt(prefs.getString("MID_Zip", null)));
        homePhone.setValue(AESEncryption.decrypt(prefs.getString("MID_HomePhone", null)));
        workPhone.setValue(AESEncryption.decrypt(prefs.getString("MID_WorkPhone", null)));


        weight.setValue(AESEncryption.decrypt(prefs.getString("MID_Weight", null)));
        height.setValue(AESEncryption.decrypt(prefs.getString("MID_Height", null)));
        primaryInsurance.setValue(AESEncryption.decrypt(prefs.getString("MID_PrimaryInsurance", null)));
        primaryInsuranceNumber.setValue(AESEncryption.decrypt(prefs.getString("MID_PrimaryInsuranceNumber", null)));
        primaryInsuranceGroupNumberOrMainPH.setValue(AESEncryption.decrypt(prefs.getString("MID_PrimaryInsuranceGroupNumberOrMainPH", null)));

        if (AESEncryption.decrypt(prefs.getString("MID_MaritalStatus", "")).equals("Single"))
            maritalStatus.setValue(new ListItem(1L, "Single"));
        if (AESEncryption.decrypt(prefs.getString("MID_MaritalStatus", "")).equals("Married"))
            maritalStatus.setValue(new ListItem(2L, "Married"));
        if (AESEncryption.decrypt(prefs.getString("MID_MaritalStatus", "")).equals("Divorced"))
            maritalStatus.setValue(new ListItem(3L, "Divorced"));
        if (AESEncryption.decrypt(prefs.getString("MID_MaritalStatus", "")).equals("Widowed"))
        ...
        if (AESEncryption.decrypt(prefs.getString("MID_Ethnicity", "")).equals("American Indian or Alaskan Native"))
            ethnicity.setValue(new ListItem(1L, "American Indian or Alaskan Native"));

        if (AESEncryption.decrypt(prefs.getString("MID_Ethnicity", "")).equals("Asian"))
            ethnicity.setValue(new ListItem(2L, "Asian"));

        if (AESEncryption.decrypt(prefs.getString("MID_Ethnicity", "")).equals("Black or African American"))
            ethnicity.setValue(new ListItem(3L, "Black or African American"));

        if (AESEncryption.decrypt(prefs.getString("MID_Ethnicity", "")).equals("Hispanic or Latino"))
            ethnicity.setValue(new ListItem(4L, "Hispanic or Latino"));

        if (AESEncryption.decrypt(prefs.getString("MID_Ethnicity", "")).equals("Native Hawaiian or other Pacific Islander"))
            ethnicity.setValue(new ListItem(5L, "Native Hawaiian or other Pacific Islander"));

        if (AESEncryption.decrypt(prefs.getString("MID_Ethnicity", "")).equals("White"))
            ethnicity.setValue(new ListItem(6L, "White"));

        if (AESEncryption.decrypt(prefs.getString("MID_Ethnicity", "")).equals("Two or more races"))
            ethnicity.setValue(new ListItem(7L, "Two or more races"));

        if (AESEncryption.decrypt(prefs.getString("MID_Ethnicity", "")).equals("Other"))
        ...
        if (AESEncryption.decrypt(prefs.getString("MID_Gender", "")).equals("Male"))
            gender.setValue(new ListItem(1L, "Male"));
        if (AESEncryption.decrypt(prefs.getString("MID_Gender", "")).equals("Female"))
            gender.setValue(new ListItem(2L, "Female"));
        if (AESEncryption.decrypt(prefs.getString("MID_Gender", "")).equals("Other"))
            gender.setValue(new ListItem(3L, "Other"));

        if (AESEncryption.decrypt(prefs.getString("MID_BloodType", "")).equals("A+"))
            bloodType.setValue(new ListItem(1L, "A+"));

        if (AESEncryption.decrypt(prefs.getString("MID_BloodType", "")).equals("A-"))
            bloodType.setValue(new ListItem(2L, "A-"));

        if (AESEncryption.decrypt(prefs.getString("MID_BloodType", "")).equals("B+"))
            bloodType.setValue(new ListItem(3L, "B+"));

        if (AESEncryption.decrypt(prefs.getString("MID_BloodType", "")).equals("B-"))
            bloodType.setValue(new ListItem(4L, "B-"));

        if (AESEncryption.decrypt(prefs.getString("MID_BloodType", "")).equals("O+"))
            bloodType.setValue(new ListItem(5L, "O+"));

        if (AESEncryption.decrypt(prefs.getString("MID_BloodType", "")).equals("O-"))
            bloodType.setValue(new ListItem(6L, "O-"));

        if (AESEncryption.decrypt(prefs.getString("MID_BloodType", "")).equals("AB+"))
        ...
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Alabama"))
            state.setValue(new ListItem(1L, "Alabama"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Alaska"))
            state.setValue(new ListItem(2L, "Alaska"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Arizona"))
            state.setValue(new ListItem(3L, "Arizona"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Arkansas"))
            state.setValue(new ListItem(4L, "Arkansas"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("California"))
            state.setValue(new ListItem(5L, "California"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Colorado"))
            state.setValue(new ListItem(6L, "Colorado"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Connecticut"))
            state.setValue(new ListItem(7L, "Connecticut"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Delaware"))
            state.setValue(new ListItem(8L, "Delaware"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Florida"))
            state.setValue(new ListItem(9L, "Florida"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Georgia"))
            state.setValue(new ListItem(10L, "Georgia"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Hawaii"))
            state.setValue(new ListItem(11L, "Hawaii"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Idaho"))
            state.setValue(new ListItem(12L, "Idaho"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Illinois"))
            state.setValue(new ListItem(13L, "Illinois"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Indiana"))
            state.setValue(new ListItem(14L, "Indiana"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Iowa"))
            state.setValue(new ListItem(15L, "Iowa"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Kansas"))
            state.setValue(new ListItem(16L, "Kansas"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Kentucky"))
            state.setValue(new ListItem(17L, "Kentucky"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Louisiana"))
            state.setValue(new ListItem(18L, "Louisiana"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Maine"))
            state.setValue(new ListItem(19L, "Maine"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Maryland"))
            state.setValue(new ListItem(20L, "Maryland"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Massachusetts"))
            state.setValue(new ListItem(21L, "Massachusetts"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Michigan"))
            state.setValue(new ListItem(22L, "Michigan"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Minnesota"))
            state.setValue(new ListItem(23L, "Minnesota"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Mississippi"))
            state.setValue(new ListItem(24L, "Mississippi"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Missouri"))
            state.setValue(new ListItem(25L, "Missouri"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Montana"))
            state.setValue(new ListItem(26L, "Montana"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Nebraska"))
            state.setValue(new ListItem(27L, "Nebraska"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Nevada"))
            state.setValue(new ListItem(28L, "Nevada"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("New Hampshire"))
            state.setValue(new ListItem(29L, "New Hampshire"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("New Jersey"))
            state.setValue(new ListItem(30L, "New Jersey"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("New Mexico"))
            state.setValue(new ListItem(31L, "New Mexico"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("New York"))
            state.setValue(new ListItem(32L, "New York"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("North Carolina"))
            state.setValue(new ListItem(33L, "North Carolina"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("North Dakota"))
            state.setValue(new ListItem(34L, "North Dakota"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Ohio"))
            state.setValue(new ListItem(35L, "Ohio"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Oklahoma"))
            state.setValue(new ListItem(36L, "Oklahoma"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Oregon"))
            state.setValue(new ListItem(37L, "Oregon"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Pennsylvania"))
            state.setValue(new ListItem(38L, "Pennsylvania"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Rhode Island"))
            state.setValue(new ListItem(39L, "Rhode Island"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("South Carolina"))
            state.setValue(new ListItem(40L, "South Carolina"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("South Dakota"))
            state.setValue(new ListItem(41L, "South Dakota"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Tennessee"))
            state.setValue(new ListItem(42L, "Tennessee"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Texas"))
            state.setValue(new ListItem(43L, "Texas"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Utah"))
            state.setValue(new ListItem(44L, "Utah"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Vermont"))
            state.setValue(new ListItem(45L, "Vermont"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Virginia"))
            state.setValue(new ListItem(46L, "Virginia"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Washington"))
            state.setValue(new ListItem(47L, "Washington"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("West Virginia"))
            state.setValue(new ListItem(48L, "West Virginia"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Wisconsin"))
            state.setValue(new ListItem(49L, "Wisconsin"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Wyoming"))
            state.setValue(new ListItem(50L, "Wyoming"));
        if (AESEncryption.decrypt(prefs.getString("MID_State", "")).equals("Other"))
            state.setValue(new ListItem(50L, "Other"));
        // Code to restore dob.


        String dobAsString = AESEncryption.decrypt(prefs.getString("MID_Birthday", null));
        if (dobAsString != null) {
